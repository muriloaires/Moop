...
import brmobi.moop.features.service.AprovarService;
...
public enum NotificationController {
    ...
    private void handleNovoMorador(RemoteMessage message, Context context) {
        ...
        aprovarIntent.setAction(AprovarService.ACTION_APROVAR);
        aprovarIntent.putExtra(AprovarService.PARAM_PERFIL_HAB_ID, message.getData().get(KEY_ID_OBJ));
        ...
        desaprovarIntent.setAction(AprovarService.ACTION_DESAPROVAR);
        desaprovarIntent.putExtra(AprovarService.PARAM_PERFIL_HAB_ID, message.getData().get(KEY_ID_OBJ));
