...
public class CondominiosFragment extends Fragment implements RotaCondominio.CondominiosHandler {
    ...
    public void onCondominiosRecebidos(List<Condominio> condominios) {
        ...
        if (condominios.size() == 0) {
            ...
            btnCadastrarCondominio.setVisibility(View.INVISIBLE);
            btnCadastrarCondominio.setClickable(false);
    ...
    public void select(int adapterPosition) {
        ...
        btnCadastrarCondominio.setClickable(true);
        if (lastSelected != -1) {
    ...
    private void loadBlocos() {
        if (lastSelected == -1) {
            ...
            ((AddCondominioActivity) getContext()).showBlocosFragment(condominios.get(lastSelected).getId(), condominios.get(lastSelected).getNome());
        }
    }
